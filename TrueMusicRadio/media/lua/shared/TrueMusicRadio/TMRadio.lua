...
TMRadio.AddOverrides = function()
	...
	function ISMoveableSpriteProps:pickUpMoveableInternal(square, object, sprInstance, spriteName, createItem, rotating)
		if isClient() then
			local item = self:instanceItem(spriteName)
			if item and instanceof(item, "Radio") then
				local deviceData = item:getDeviceData()
              	    		if deviceData and object and object.getX and object:getX() then
		...
		return TMRMoveableSpriteProps_pickUpMoveableInternal(self, square, object, sprInstance, spriteName, createItem, rotating)
	end
end
