...
TMRadio.AddOverrides = function()
	...
	function RWMGeneral:setInfoLines()
		...
		if self.deviceData and TMRadio.mod.options.enableRDS then
			if not self.deviceData:getIsTelevision() then
				local channel = self.deviceData:getChannel()
				if self.deviceData:getIsTurnedOn() and (channel == 94000 or channel == 94200 or channel == 94400 or channel == 94600 or channel == 94800) then
					local songName = nil
					...
					elseif channel == 94400 or channel == 94600 or channel == 94800 then
 				   		songName = TMRadio.PlaylistGlobal[TMRadio.Channels[channel]]
					end
					if songName then
	...
	for index = #TMRadio.soundCache, 1, -1 do 
    	...
    	for index,t in ipairs(TMRadio.soundCache) do   
        	...
        	if TMRadio.isPlaying(t) or (t.deviceData:getParent() ~= nil and t.deviceData:getIsTurnedOn()) then
				...
				if TMRadio.mod.options.muteMusic then
					t.sound:setVolume(0)
            			elseif t.deviceData:isInventoryDevice() then
                			highestVolume = 1
...
TMRadio.translation = {
	muteMusic = getText("UI_TrueMusicRadio_MO_muteMusic"),
	muteMusicTooltip = getText("UI_TrueMusicRadio_MO_muteMusic_tooltip"),
	enableRDS = getText("UI_TrueMusicRadio_MO_enableRDS"),
	enableRDSTooltip = getText("UI_TrueMusicRadio_MO_enableRDS_tooltip"),
	update94 = getText("UI_TrueMusicRadio_update94"),
	update942 = getText("UI_TrueMusicRadio_update942"),
	revert94 = getText("UI_TrueMusicRadio_revert94"),
	revert942 = getText("UI_TrueMusicRadio_revert942"),
...
TMRadio.mod = {
    	...
    	names = {
		muteMusic = TMRadio.translation.muteMusic,
		enableRDS = TMRadio.translation.enableRDS
	},
    	mod_id = "TrueMusicRadio",
    	mod_shortname = "True Music Radio"
}

TMRadio.loadModOptions = function()
    	if ModOptions and ModOptions.getInstance then
        	TMRadio.modSettings = ModOptions:getInstance(TMRadio.mod)
        	local muteMusic = TMRadio.modSettings:getData("muteMusic")
		muteMusic.tooltip = TMRadio.translation.muteMusicTooltip
        	local enableRDS = TMRadio.modSettings:getData("enableRDS")
		enableRDS.tooltip = TMRadio.translation.enableRDSTooltip
	end
end

TMRadio.loadModOptions()